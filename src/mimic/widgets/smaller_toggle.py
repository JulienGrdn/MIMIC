...
class Toggle(QCheckBox):
    ...
    def __init__(self,
        ...
        max_width=40  # New max width parameter
        ):
        ...
        self.setMaximumSize(QSize(max_width, 35))  # Max width of 30px

        self.stateChanged.connect(self.handle_state_change)

    def sizeHint(self):
        return QSize(40, 35)  # Small width (Max width matches this)
    ...
    def paintEvent(self, e: QPaintEvent):
        ...
        handleRadius = round(0.35 * contRect.height())  # Slightly larger handle
        ...
        barRect = QRectF(
            0, 0,
            min(contRect.width() - 2 * handleRadius, 30),  # Ensuring max width constraint
            0.40 * contRect.height()
        )
        
        # --- FIX: explicit cast to QPointF ---
        barRect.moveCenter(QPointF(contRect.center()))
        
        rounding = barRect.height() / 2

        trailLength = min(contRect.width() - 2 * handleRadius, 30)  # Limit movement range
...
class AnimatedToggle(Toggle):
    ...
    def set_state(self, value: bool):
        ...
        self.handle_state_change(1 if value else 0)
